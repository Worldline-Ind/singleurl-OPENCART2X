...
class ControllerPaymentPaynimo extends Controller {
	public function index() {
		...
		$cur_date = date("d-m-Y");
		$returnUrl = $this->url->link('payment/paynimo/getResponse');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paynimo.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/paynimo.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/paynimo.tpl', $data);
		}
	}
	...
	public function confirm() {
        if ($this->session->data['payment_method']['code'] == 'paynimo') {
			...
			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('paynimo_order_status'));
		}
	}
}